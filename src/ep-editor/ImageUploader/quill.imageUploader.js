...
class ImageUploader {
    ...
    handleDrop(evt) {
        ...
        if (
            ...
            setTimeout(() => {
                this.range = this.quill.getSelection();
                // 删除vue未禁用的drag事件
                this.quill.deleteText(this.range.index-1, 1, "user");
    ...
    insertToEditor(url) {
        ...
        // Insert the server saved image   -- nei cun yinyong
        this.delta = this.quill.insertEmbed(range.index, "image", `${url}`, "user");
        range.index++;
