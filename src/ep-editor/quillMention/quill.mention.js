...
import {registerMentionBlot} from "./blots/mention";
...
class Mention {
    ...
    insertItem(data, programmaticInsert, overriddenOptions = {}) {
        ...
        if (options.spaceAfterInsert) {
            this.quill.insertText(insertAtPos + 1, "\n", Quill.sources.USER);
            // setSelection here sets cursor position
            this.quill.setSelection(insertAtPos + 1,0, Quill.sources.USER);
        } else {
...
export function registerMention(Quill) {
